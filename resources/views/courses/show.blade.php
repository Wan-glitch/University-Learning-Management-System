                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-start justify-content-between">
                                <div class="d-flex align-items-start">
                                    <img src="{{ $announcement->user->profile_photo_url ?? 'default_profile_photo_url' }}" alt="user" class="rounded-circle me-3" width="40" height="40">
                                    <div>
                                        <h5 class="card-title">{{ $announcement->user->name }}</h5>
                                        <h6 class="card-subtitle text-muted">{{ $announcement->created_at->format('d M Y') }}</h6>
                                    </div>
                                </div>
                                <div class="dropdown">
                                    <button class="btn btn-link text-dark" type="button" id="dropdownMenuButton{{ $announcement->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                        <i data-feather="more-vertical"></i>
                                    </button>
                                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton{{ $announcement->id }}">
                            ...
                            @if($announcement->photo)
                                <img src="{{ asset('storage/' . $announcement->photo) }}" alt="Photo" class="img-fluid mt-3">
                            @endif
                            ...
                            @endif
                                <div class="comments-section mt-3">
                                    ...
                                    @elseif($announcement->comments->count() > 1)
                                        <a href="#" class="show-comments" data-announcement-id="{{ $announcement->id }}">{{ $announcement->comments->count() }} comments</a>
                                    @endif
                                    <div class="d-flex mt-3">
                                        <img src="{{ auth()->user()->profile_photo_url ?? 'default_profile_photo_url' }}" alt="user" class="rounded-circle me-2" width="40" height="40">
                                        <input type="text" class="form-control comment-input" placeholder="Write a comment..." data-announcement-id="{{ $announcement->id }}">
                                        {{-- <button type="button" class="btn btn-primary post-comment-button" data-announcement-id="{{ $announcement->id }}">Post Comment</button> --}}
                                        <button type="button" class="btn btn-primary post-comment-button" style="margin-left: 10px;" data-announcement-id="{{ $announcement->id }}">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="d-flex mt-3">
                    <img src="{{ auth()->user()->profile_photo_url ?? 'default_profile_photo_url' }}" alt="user" class="rounded-circle me-2" width="40 " height="40">
                    <input type="text" class="form-control modal-comment-input" placeholder="Add class comment...">
                    {{-- <button type="button" class="btn btn-primary post-modal-comment-button">Post</button> --}}
                    <button type="button" class="btn btn-primary post-modal-comment-button" style="margin-left: 10px;">
                        <i data-feather="send" class="feather-icon"></i>
                    </button>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.show-comments').on('click', function(event) {
            ...
            var announcementId = $(this).data('announcement-id');
            $.ajax({
                url: "{{ route('comments.show', ':announcementId') }}".replace(':announcementId', announcementId),
                type: 'GET',
                success: function(response) {
